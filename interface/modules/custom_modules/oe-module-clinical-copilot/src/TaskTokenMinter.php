 ...
 * side. The signing key is sourced from {@see ModuleSettings::KEY_JWT_SIGNING_KEY};
 * the sidecar reads the same value from ``COPILOT_BFF_JWT_SIGNING_KEY``.
 *
 * @package   OpenEMR
    ...
    public function __construct(private readonly string $signingKey)
    {
        if ($signingKey === '' || $signingKey === 'change-me-to-a-32-byte-hex-string') {
            throw new TaskTokenConfigurationError(
                ...
                . 'signing key. Open the Clinical Co-Pilot admin page and '
                . 'click "Generate" on the JWT Signing Key field; then '
                . 'mirror the same value into the sidecar\'s '
                . 'COPILOT_BFF_JWT_SIGNING_KEY environment variable.'
            );
        }
    }
