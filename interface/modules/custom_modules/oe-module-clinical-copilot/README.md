...
A pairwise AI diagnostic cross-check and chart-error scan for OpenEMR. Read-only against FHIR. Citations required for every answer. HIPAA-aligned audit trail.

## What it does

- **Diagnostic cross-check.** Ranks candidate diagnoses pairwise (every candidate vs. every other) instead of asking the LLM to produce a list. Surfaces the top three with citations to FHIR resources in the patient's chart.
...
Sidecar (FastAPI + LangGraph + Postgres)
   ▼ (FHIR fan-out)
OpenEMR FHIR API ───────────────────────▶ Pairwise engine ─[ LLM (OpenAI Enterprise BAA) ]
                                             │
...
- [DEVELOPER_GUIDE.md](DEVELOPER_GUIDE.md) — extending the rule store and adding new pairwise prompts.
