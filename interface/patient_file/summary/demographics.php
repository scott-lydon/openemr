...
// Note: the modern Next.js patient dashboard used to be embedded in
// the OpenEMR iframe via a redirect block here. That approach hit
// browser security limits (mixed-content blocking in Safari/Firefox,
// cookie SameSite issues with reverse-proxied iframes, Auth.js v5 +
// Next.js basePath incompatibilities). We backed it out: the modern
// dashboard is now a standalone top-level app at
// `patient_dashboard_modern_url` that the clinician opens in a new
// tab via the "Open Modern Dashboard" link in the patient header
// (interface/main/tabs/templates/patient_data_template.php). No
// iframe embedding, no SMART-EHR-launch silent skip — the dashboard
// goes through the standard OAuth2/OIDC dance with a login at the
// authorization server. Same model as the Clinical Co-Pilot.
