...
4. **A new internal Read API** under `interface/modules/custom_modules/oe-module-clinical-copilot/src/Rest/SnapshotController.php` that returns the denormalized snapshot in one call, signed by the BFF. This wraps the FHIR fan-out with the reconciliation pass.
