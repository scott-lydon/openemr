 ...
 *     SMART system scopes, the redirect URI, the grant_types value,
 *     and the client name are all constants here. The shell wrapper
 *     does not pass them in, so they cannot drift.
 ...
 * `clinical-copilot/sidecar/openemr_oauth.py` uses HTTP Basic auth on
 * the OAuth2 `/token` endpoint with this client_id and client_secret,
 * so we register `client_credentials` as the grant_type and the
 * `system/*.read` scopes the sidecar's FHIR client actually requests.
 *
 * @package   OpenEMR
...
final class ProvisionClinicalCoPilotApiClientCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        try {
            ...
            // 1. Find every existing row whose client_name matches ours
            //    and remove it. Counts toward the `rotated` flag in the
            //    JSON output so the shell wrapper can tell the user
            //    whether they got a fresh registration or a rotation.
            $existing = [];
            ...
            // 2. Insert a fresh row. ClientRepository::insertNewClient()
            //    encrypts the client_secret with the standard CryptoGen
            //    pipeline, so we hand it the plaintext here and the
            //    column receives the encrypted form. We keep the
            //    plaintext in $clientSecret to print to stdout.
            ...
            // 3. Force enable. ClientRepository::insertNewClient() sets
            //    is_enabled=0 for any client that requests system/*
            //    scopes (see ScopeRepository::hasScopesThatRequireManualApproval),
            //    on the assumption that an admin will approve. We are
            //    the admin in this provisioning context, so flip it on.
            ...
            // 4. Emit one JSON line on stdout. The shell wrapper parses
            //    it with python -c 'import json; ...'; no Symfony table
            //    quirks, no whitespace heuristics. Any extra text the
            //    framework writes (deprecation warnings, etc.) goes to
            //    STDERR by Symfony Console convention so it never
